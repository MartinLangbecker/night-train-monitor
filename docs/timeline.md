...
- **23.09.** Verkaufsstart und Beginn des Scrapings **ab Tag 1** — seltener Glücksfall: die volle Fill-Kurve ab Verkaufsöffnung wird erfasst (bei allen anderen Providern begann die Erhebung mitten im Verkauf, Start-Kapazität verzerrt).
  ...
  - **Wagenmaterial / Kapazitätsdeutung revidiert** (Quelle: InnoTrans-Mock-up-Sichtung + PM, NICHT aus der API): Zum Start fahren "Nox Suites" in **1:1-Anordnung** (ein Sitz je Fensterreihe, 2 m Liegelänge in Lounge-Position). Der gezeigte Wagen ist ein **Bpmz 294.2** (ehem. IC-2.-Klasse-Großraum, regulär ~80 Sitze). In 1:1-Suite-Umbau passen deutlich weniger Plätze hinein — **`total=58` ist damit plausibel EIN Suite-Wagen**, nicht die halbe Kapazität eines größeren. Folglich ist der Sprung auf `total=116` ab ~14./15.04. das Ankuppeln eines **zweiten** Suite-Wagens (2×58), keine bloße Kontingenterhöhung. (Frühere Deutung "1 Wagen = halbe Kapazität" damit präzisiert.)
